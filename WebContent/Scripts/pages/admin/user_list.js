...
function openEditUser(id){
			alert(id);
			$('#editUser').window({href:'get_user?userId='+id});
            ...
            $.ajax({
			  ...
			  success : function(data){
					  ...
					  $.each( data.Rows  , function(commentIndex, comment) {
						...
						var strhtml="<a href=\"#\" onclick=\"openEditUser($(this).next().html())\">编辑</a><span style=\"display:none;width:10px\">"+comment['userId']+"</span><span style='display:none'>"+i+"</span><span style=\"display:inline-block;width:10px\"></span><a href=\"#\" onclick=\"deleteRow($(this).parent().parent().parent(),$(this).prev().prev().prev().html())\">删除</a>";
